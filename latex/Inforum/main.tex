...
A type is either a type constructor $\tiota$, a type variable $\talpha$, an abstraction $\tabs{\talpha}{\kind}{\typec{T}}$ or an application $\tapp{\typec{T}}{\typec{T}}$. A detailed list of types and constructors is in Figure \ref{fig:syntax-types}. Note that the last three constructors are actually type constants. Observe also that $\tmuinfix{\alpha}{\kind}{T}$ is syntactic sugar for $\typec{\tmu\kind (\tlambda{\alpha}{\kind}{T})}$ and likewise $\tquantinfix\alpha\kind \TT$ is an abbreviation for $\tforall\kind(\tlambda{\alpha}{\kind}{T})$ or $\texists\kind(\tlambda{\alpha}{\kind}{T})$.
...
[EXEMPLOS lambda e mu]. Only types that normalise shall be well-formed.

Finally, we introduce the concept of \emph{type formation}. We denote this as $\judgementrelctx{\Delta}{\typec{T}}{\colon}{\kind}$, that is, $\typec{T}$ is \emph{well-formed} with kind $\kind$ under the kinding context $\Delta$, a map from type variables to kinds. The rules for type formation can be found in Figure \ref{fig:type-formation}.
...
In section \ref{sec:deciding-type-formation} we prove the decidability of type formation. 
...
The notion of renaming we follow can be found in Costa et al \cite{PocasCMV23}.
There are three possible ways to check whether two (renamed) types are equivalent, according to Costa et al \cite{PocasCMV23}:
\begin{itemize}
    \item by a set of coinductive rules;
    \item by means of type bisimulation, built from a labelled transition system;
    \item by translating types to grammars and checking bisimulation of grammars.
\end{itemize}

We follow the third approach and go into detail in section \ref{sec:deciding-type-equivalence}.
...
%Two types \typec{T} and \typec{U} are \emph{bisimilar}, written $\typec{T} \sim \typec{U}$, if there is a bisimulation $\mathcal{R}$ such that $(\typec{T},\typec{U})\in\mathcal{R}$. For example, consider type $\typec{T} = \tapp{\talpha}{(\INp \Bool)}$ and $\typec{U} = \seqcomp{\tapp{\talpha}{(\INp \Bool)}}{\Skip}$. We show that $\typec{T}\sim\typec{U}$.
