...
If $\typec{T'_i}$ does not reduce by any $\mu$-reduction, we can determine that $\typec{T}$ normalises. Otherwise, since $\tmu\kast\ \UT$ is restricted to a base kind $\kast$, it must reduce by one of these:
